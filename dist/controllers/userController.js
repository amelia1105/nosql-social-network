...
export const deleteUser = async (req, res) => {
    try {
        ...
        await User.updateMany({ friends: { $in: [new ObjectId(req.params.userId)] } }, { $pull: { friends: new ObjectId(req.params.userId) } });
...
export const addFriend = async (req, res) => {
    try {
        const user = await User.findByIdAndUpdate({ _id: req.params.userId }, { $addToSet: { friends: req.params.friendId } }, { new: true, runValidators: true });
        if (!user) {
            return res.status(404).json({
...
export const removeFriend = async (req, res) => {
    try {
        const user = await User.findByIdAndUpdate({ _id: req.params.userId }, { $pull: { friends: req.params.friendId } }, { new: true, runValidators: true });
        if (!user) {
            return res.status(404).json({
                message: 'No user with this id.'
            });
